...
\newglossaryentry{rest}{
    name=REST,
    description={Representational State Transfer, ein Architekturstil für Webservices bzw. APIs}
}
...
% Abstract
    \begin{abstract}
        ...
        Die Anwendung umfasst eine Befehlszeilenschnittstelle (CLI) und einen Hintergrundprozess (Daemon), der in regelmässigen Abständen Snapshots der überwachten Verzeichnisse erstellt.
...
% Main Content
    ...
    \subsection{Chance}\label{subsec:chance}\index{Produktivitätssteigerung}
    Durch das Angebot an generativen KI-Modellen\index{KI-Modell}, die - meist frei oder relativ günstig - im
    ...
    Des Weiteren lassen sich Schlüsse aus Änderungen, Löschungen und Neuerstellungen von Dateien ziehen.\footnote{Evaluation durch KI-Modell in Bezug auf die Bedeutung von Änderungen, Löschungen und Neuerstellungen nicht weiter realisiert, siehe Abschnitt \nameref{sec:zukunftigearbeiten} bzw. \nameref{subsec:funktionale-erweiterungen}.}
    Durch das automatisierte Abfragen dieser KI-Modelle in einer eigenen Anwendung,
    welche unter anderem Momentaufnahmen (sogenannte\ Snapshots\index{Snapshots}) relevanter Dateipfade und Metadaten speichert,
    ...
    \section{Projektziel}\label{sec:projektziel}\index{Projektziel}
    ...
    Diese periodische Überwachung soll in einem Hintergrundprozess, einem so genannten \gls{daemon}\index{Daemon}, stattfinden.
    ...
    Die Kommunikation zwischen CLI\index{CLI} und Daemon erfolgt über eine\index{REST-API} \gls{rest}-\gls{api}\@.
    ...
    Sie befindet sich lokal auf dem System in Form einer \gls{sqlite}-Datenbank\index{SQLite}.
    Die Wahl des \gls{dbms} sowie dessen genaue Implementierung wird in den Kapiteln~\nameref{subsubsec:technologieentscheid} und~\nameref{subsubsec:realisierung-datenbank} beschrieben.

    \subsubsection{KI-Anbindung}\label{subsubsec:spezifikation-ki-anbindung}
    Es wurde das KI-Modell\index{KI-Modell} \textit{\gls{gpt}-4o mini} von \gls{openai} verwendet.
    ...
    Alle funktionalen Anforderungen werden in Form von User Stories im Scrum\index{Scrum} Product Backlog geführt.
    ...
    \begin{itemize}
        \item Das System findet jegliche Dateien (insbesondere Cache- und Logdateien\index{Cache- und Logdateien}), welche sich in einem angegebenen Verzeichnis (inkl.\ Unterverzeichnisse) befinden.
    ...
    \begin{itemize}
        ...
        \item Minimales Testing\index{Testing}-Setup, um den Entwicklungsaufwand gering zu halten.
...
test.log CHANGED 7-times
    ...
    \gls{sql}-Datenbanken basieren auf einem relationalen Modell mit festen Schemata.\index{SQLite}
    ...
    Er speichert periodisch Snapshots\index{Snapshots} von überwachten Pfaden in der Datenbank (1.1)\footnote{Diese Nummerierung bezieht sich auf das nachfolgende Sequenzdiagramm}.
...
<Inhalt der Datei>
    ...
    In diesem Kapitel wird auf die Verwendung der Projektmethode Scrum\index{Scrum} während des Projekts eingegangen.
    ...
    \chapter{Evaluation und Tests}\index{Testing}
...
monitoring database.
    ...
    Generative KI-Modelle\index{KI-Modell}, wie die GPT-Modelle von OpenAI sind in der Lage unstrukturierte Daten zu analysieren und Einschätzungen in Form von Text auszugeben.
...
index forbidden by rule: /var/www/html/
    ...
    Angenommen die System-Prompt und Completion soll 2'000 Tokens lang sein, so könnte eine Logdatei mit maximal \sfrac{128'000 - 2'000}{40} = 3'150 Zeilen analysiert werden.
    ...
    \textbf{Für alle nachfolgenden Schritte wird folgendes vorausgesetzt:}
    \begin{itemize}
        \item \textbf{Eine Installation von Java 21+ (\gls{jre}).}
        \item \textbf{Eine aktuelle Maven-Installation (insofern manuell kompiliert wird).}
        \item \textbf{Das zip-Archiv wurde am gewünschten Installationsort entpackt.}
    \end{itemize}

    \subsection{OpenAI API Key}
    Wenn Sie die Inspektionsfunktion nutzen möchten, erstellen Sie einen OpenAI-API-Schlüssel, indem Sie den Anweisungen unter \url{https://platform.openai.com/settings/organization/api-keys} folgen.
    ...
    \begin{itemize}
        \item \texttt{build} Kompiliert das Projekt, erstellt die ausführbaren JAR-Dateien und ersetzt diese im \texttt{bin}-Ordner.
        \item \texttt{setup} Erstellt notwendige Umgebungsvariablen und richtet den Autostart des Daemons ein.
        \item \texttt{build-setup} Sequenzielle Ausführung von \texttt{build} und \texttt{setup}.
    \end{itemize}
    Da die JAR-Dateien bereits im Installationsverzeichnis enthalten sind, ist ein \textbf{manuelles Kompilieren nicht notwendig}.
    Es wird im Folgenden empfohlen, direkt \texttt{setup} auszuführen.
    Damit der OpenAI API Key von den Skripten erkannt wird, muss er sich in einer Datei namens \texttt{apikey.txt} im Installationsverzeichnis befinden.

    \subsubsection{Linux}
    Folgende Skripte sind für Linux verfügbar:
    % TODO: Ammann wie unten die Listings einfügen

    \subsubsection{Windows}
    Folgende Skripte sind für Windows verfügbar:
    \begin{itemize}
        \item \texttt{build.ps1}
        \item \texttt{setup-win.ps1}
        \item \texttt{build-setup-win.ps1}
    \end{itemize}

    Im Vorfeld muss sichergestellt werden, dass die Skripte ausgeführt werden können.
    \textbf{Achtung}: hierbei wir die Ausführung von allen lokalen Skripten erlaubt.
    Diese Aktion kann nur von Administratoren durchgeführt werden und wird mit folgendem Befehl in PowerShell erreicht:
    \begin{lstlisting}[label={lst:lstlisting-windows-inst-1},caption={Windows Berechtigungen}]
        set-executionpolicy remotesigned
    \end{lstlisting}

    Nachfolgend die empfohlene Ausführung von \texttt{setup-win.ps1}:
    \begin{lstlisting}[label={lst:lstlisting-windows-inst-2},caption={Windows Installation}]
        .\setup-win.ps1
    \end{lstlisting}
    ...
    \begin{itemize}
        \item \texttt{build}
        \item \texttt{setup-mac}
        \item \texttt{build-setup-mac}
    \end{itemize}

    \textbf{Achtung}: Der Einfachheit halber werden ausschliesslich Konfigurationen im \texttt{$\sim$/.zprofile} File vorgenommen.
    Diese können bei Bedarf manuell entfernt werden.
    Das \texttt{setup} Skript sollte nur einmal ausgeführt werden, um doppelte Einträge im \texttt{$\sim$/.zprofile} zu vermeiden.

    Im Vorfeld muss sichergestellt werden, dass die CLI und die Skripte ausgeführt werden können.
    Dies wird mit folgendem Befehl erreicht:
    \begin{lstlisting}[label={lst:lstlisting-macos-2},caption={MacOS Berechtigungen}]
        chmod +x ts
    ...
    \begin{lstlisting}[label={lst:lstlisting-macos-3},caption={MacOS Quarantäne-Tag}]
        xattr -d com.apple.quarantine <skript>
    \end{lstlisting}

    Nachfolgend die empfohlene Ausführung von \texttt{setup-mac}:
    \begin{lstlisting}[label={lst:lstlisting-macos-1},caption={MacOS Installation}]
        ./setup-mac
    \end{lstlisting}


    \clearpage
    \subsection{Manuelle Installation}\label{sec:manuelle-installation}
    ...
    \subsubsection{Linux}\label{subsec:linux}
    \begin{enumerate}
...
PATH=$PATH:$TRACE_SENTRY_DIR
        \end{lstlisting}
        \item Setzen Sie anschliessend den generierten API-Schlüssel als Umgebungsvariable am Ende der Datei \texttt{/etc/profile}:
...
[System.Environment]::SetEnvironmentVariable("Path", "$currentPath;<absoluter Pfad zum Installationsverzeichnis>","User")
        \end{lstlisting}
        \item Setzen Sie anschliessend den generierten API-Schlüssel als Umgebungsvariable via PowerShell:
...
Copy-Item "$env:TRACE_SENTRY_DIR\ts-daemon.bat" "$env:APPDATA\Microsoft\Windows\Start Menu\Programs\Startup"
    ...
    \subsubsection{macOS}\label{subsec:macos}
...
echo export PATH="\$TRACE_SENTRY_DIR:\$PATH" >> ~/.zprofile
        \end{lstlisting}
        \item Setzen Sie anschliessend den generierten API-Schlüssel als Umgebungsvariable im \texttt{.zprofile} File:
...
echo ts run >> ~/.zprofile
    ...
    \subsection{Berechnung der Produktivitätssteigerung}\label{subsec:berechnung-der-produktivitatssteigerung}\index{Produktivitätssteigerung}
    ...
    \begin{itemize}
        ...
        \item Das Analysieren einer Datei mit GPT-4o mini kostet gemäss~\nameref{sec:ki-fallstudie} 0.011 USD $\approx$ 0.01 CHF.
