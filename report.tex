...
% Main Content
    ...
    \subsection{Unix}
    \begin{enumerate}
        \item Download the latest .zip artifact from this repository.
        \item Unpack the archive to your desired installation directory for the TraceSentry.
        \item Open a terminal and set the following environment variables:
...
export PATH=$PATH:$TRACE_SENTRY_DIR/bin
        ...
        \item If you want to use the inspect feature create an openapi API-KEY following these instructions: %todo
...
export TRACE_SENTRY_API_KEY=<generated API-KEY>
        ...
        \item If you want that the daemon starts every time automatically in the background when you start your system,
        you can do the following in a terminal:
        \begin{lstlisting}[label={lst:lstlisting-unix-3}]
crontab -e
// Add this line to the opened file:
@reboot $TRACE_SENTRY_DIR/bin/daemon
        \end{lstlisting}
        Then save the file.
    \end{enumerate}
...
copy %TRACE_SENTRY_DIR%\bin\daemon.bat "%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup"
        \end{lstlisting}
    \end{enumerate}
